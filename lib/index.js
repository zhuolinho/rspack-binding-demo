      ...
      compiler.hooks.done.tap("MyBannerPluginWrapper", () => {
        ...
        setTimeout(() => {
          try {
            ...
            if (callbackData) {
              // Create next function that resumes execution
              const next = () => {
                binding.resumeExecution();

                // Re-trigger compilation to continue with module movement
                if (this.compiler) {
              ...
              // Create addNewChunk function that can be called from callback
              const addNewChunk = (chunkName, modulePaths) => {
                return binding.addNewChunk(chunkName, modulePaths);
              };

              // Create removeModuleFromChunk function that can be called from callback
              const removeModuleFromChunk = (chunkName, modulePaths) => {
                return binding.removeModuleFromChunk(chunkName, modulePaths);
              };

              // Get chunks data
              const chunksData = binding.getChunksData();

              // Call the callback with movedModules, chunks, next function, addNewChunk function, and removeModuleFromChunk function
              this.callback(
                callbackData,
                chunksData || [],
                next,
